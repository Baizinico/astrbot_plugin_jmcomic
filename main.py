...
class JMComicDownloader(Star):
    ...
    async def process_jm_download(self, event: AstrMessageEvent, jm_id: str):
        try:
            ...
            if "pdf" in send_mode.lower():
                await self.send_pdf(event, jm_id)
            else:
    ...
    async def send_pdf(self, event: AstrMessageEvent, jm_id: str):
        try:
            ...
            if not base_path.exists():
                await event.send(event.plain_result(f"本子不存在或者下载失败力"))
                return
            ...
            if not image_paths:
                await event.send(event.plain_result("下载失败力"))
                return
            ...
            await event.send(event.plain_result(f"发送PDF时出错: {str(e)}"))
